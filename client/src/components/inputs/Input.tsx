...
import { greenTick, redCross } from "../../assets";
import { CustomInputProps } from "../../types";

export default function Input({ name, label, type = "text", value, placeholder, onChange, error, success, classes, errLabel=true, ...rest }: CustomInputProps & InputHTMLAttributes<HTMLInputElement>) {
    return (
        <div className={`${classes} flex flex-col w-full gap-3.5`}>
            {
                label &&
                <label htmlFor={name} className="text-xl leading-[25.697px] ">
                    {label}
                </label>
            }
